...
These special `__...__` methods, by the way, are called "dunder" methods ("Double UNDERscore"). And there's even more we can do with them. What if we could use `+` on two `Rectangle`s to find the smallest `Rectangle` containing them both?
```python
class Rectangle:
    def __init__(self, width, height):
        self.width = width
